...
def get_order(order_id):
    with db.connect() as con:
        return con.execute("SELECT * FROM orders WHERE order_id = ?", (order_id,)).fetchone()
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cat:"):
        ...
        for pid, p in PRODUCTS.items():
            if p["category"] == cat:
                rows.append([InlineKeyboardButton(f"{p['name']} — ${p['price']:.2f}", callback_data="product:" + pid)])
    ...
    if data.startswith("approve:"):
        ...
        await q.edit_message_text(f"✅ Order #{order_id} approved.")
        return
...
async def orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        return
    await update.message.reply_text("📦 ADMIN ORDERS\n\nPayment reports appear in Telegram when customers press 'I Have Paid'.")
